...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (!feature.Properties.Any(p => p.Key == MapElement.highway && MapFeature.HighwayTypes.Any(v => p.Value.StartsWith(v))))
        {
            ...
            else if (feature.Properties.Any(p => p.Key == MapElement.boundary && p.Value.StartsWith("forest")))
            {
                var coordinates = feature.Coordinates;
            ...
            else if (feature.Properties.Any(p => p.Key == MapElement.landuse && (p.Value.StartsWith("forest") || p.Value.StartsWith("orchard"))))
            {
                var coordinates = feature.Coordinates;
                var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
                baseShape = geoFeature;
